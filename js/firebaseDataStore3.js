(function() {
	window.firebaseDataStore3 = function(apiKey, storageID) {
		...
		var _application,
			...
			_tokenGenerator,
			_url = (function() {
				var name = (!isNaN(parseInt(storageID))) ?
					"glowing-fire-{0}".format(storageID) :
					storageID;

				return {
					"authDomain": "{0}.firebaseapp.com".format(name),
					"rootURL": "https://{0}.firebaseio.com/".format(name),
					"storageBucket": "{0}.appspot.com".format(name)
				}
			})(),
			_username;
	...
	window.firebaseDataStore3.cleanup = function(interval) {
	...
	window.firebaseDataStore3.getKey = function(name) {
